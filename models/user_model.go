...
func (model UserModel) FindUserByID(id int) (entities.User, error) {
	var user entities.User
	query := `
		SELECT email, name, photo FROM users WHERE id = ?
	`
...
func (model UserModel) GetUserPhotoByID(id int) (*string, error) {
	var photo *string
...
func (model UserModel) UpdateProfile(user entities.User) error {
    ...
    if user.Password != "" {
        query = `
            UPDATE users SET name = ?, email = ?, password = ?, photo = ?, updated_at = ? WHERE id = ?
        `
        args = []interface{}{user.Name, user.Email, user.Password, user.Photo, time.Now(), user.Id}
    } else {
        query = `
            UPDATE users SET name = ?, email = ?, photo = ?, updated_at = ? WHERE id = ?
        `
